...
import { FaUser, FaEnvelope, FaLock, FaGoogle, FaFacebookF, FaEye, FaEyeSlash, FaArrowRight } from 'react-icons/fa';
...
const LoginRegister = () => {
  ...
  // 2. Add state to track user input
  ...
  const handleLogin = (e) => {
    ...
    if (isLogin) {
        if (email === adminEmail && password === adminPass) {
            // 1. SAVE LOGIN STATUS TO BROWSER STORAGE
            localStorage.setItem('isAdmin', 'true'); 
            
            alert("Login Successful!");
            navigate('/admin'); 
            
            // Optional: Force a page reload so Navbar updates immediately
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[600px]">
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 bg-white flex flex-col items-center justify-center">
            ...
            {/* 5. Add onSubmit to form */}
            <form className="w-full space-y-5" onSubmit={handleLogin}>
                
                {!isLogin && (
                    <div className="relative">
                        <FaUser className="absolute top-1/2 -translate-y-1/2 left-4 text-gray-400" />
                        <input type="text" placeholder="Username" className="w-full bg-gray-50 pl-12 pr-4 py-3.5 rounded-xl outline-none focus:ring-2 focus:ring-[#6A4DF4]/20 border border-gray-100 focus:border-[#6A4DF4] transition-all text-sm font-medium" />
                    </div>
                )}
